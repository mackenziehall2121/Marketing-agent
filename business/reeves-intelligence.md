# Reeves Intelligence: Business Profile

Read this before doing any marketing work for Reeves. It holds the facts the founder has confirmed, the claims we are not allowed to make, and the questions still open.

## What it is
Reeves Intelligence helps nonprofits **find grant opportunities, assess whether they fit their mission and eligibility, and keep the strongest prospects organized**. It works alongside nonprofit staff. Reeves makes the research and tracking easier, and **the people who know the organization decide which grants to pursue**.

## Who it's for
Nonprofit teams that seek grant funding, especially:
- Executive directors
- Development staff
- Grant writers
- Small teams where one person handles several roles (the sharpest target: they feel the time cost most)

## The problem (before)
Finding grant listings is easy. Working out which ones the organization actually qualifies for and should spend time on takes hours of research. For each promising listing, staff have to check whether the funder supports their kind of work, serves their area, and is worth an application. Prospects end up scattered across spreadsheets, inboxes and memory.

## The result (after)
- A clearer view of which grants are worth pursuing
- One place where promising opportunities stay organized
- Less time spent sorting through poor fits
- More confidence about where to focus
- The team still makes the call. Reeves supports their judgment.

## The feeling we sell
Having time for the grants that matter and confidence that the time is well spent, instead of the dread of another afternoon lost to listings they don't qualify for. Every step ends in more funding for the mission they got into this work to serve.

## Pricing
All tiers: 7-day free trial. Signup: https://getreeves.ai/signup.html · Walkthrough video: https://youtu.be/MdZReSzp5X4

| Tier | Annual billing | Month to month | Includes |
|---|---|---|---|
| **Core** | $199/mo ($2,388/yr) | $238/mo | Discovery and mission matching; review and pursuit pipeline; writer scheduling; up to 2 users |
| **Pro** | $399/mo ($4,788/yr) | $438/mo | Everything in Core; broader discovery reach; funding intelligence reports; priority support |
| **Max** | $899/mo ($10,788/yr) | $938/mo | Everything in Pro; forecasting and advanced intelligence; up to 15 users; high-touch onboarding |

Revenue math (annual plans): $1M/yr ≈ 419 Core, or 209 Pro, or 93 Max customers.

## Proof status (hard rules)
- Reeves is **working toward its first sales**. There are no verified customer results, testimonials or performance numbers yet.
- **Never** present sales targets, projections or hypothetical results as proof.
- **Never** invent testimonials, customer counts, time-saved figures or grant-win rates.
- Until real proof exists, earn trust with honesty, a strong guarantee, genuinely useful free content, and founder credibility.

## Competition and positioning
Prospects currently rely on grant databases, consultants, spreadsheets, or their own manual research.
Intended difference: Reeves helps teams **assess fit and manage the opportunities they choose to pursue**, rather than handing them a list of search results.
- Do **not** make specific competitor comparisons or claims of superiority until they are researched and verified against the product.

## Current state
- Website: getreeves.ai
- Actively building prospect lists and outreach for first sales
- Nonprofit research underway in **West Virginia** and **Los Angeles**
- Time available: 25 hours/week, more if needed
- Email list: ~20,000 contacts. **Researched prospect list of nonprofits (cold, not opted in).** Treat it as cold B2B outreach, not a newsletter list: separate sending domain, small daily volume, plain-text personal emails, clear opt-out, and move people onto the real (opted-in) list only when they reply or sign up.
- Ad budget: $0

## Open questions (ask before writing on these topics)
- Founder story: Reeves was created by the founder's **dad**. A real story exists, but it is **on hold**. Do not write or hint at a founder story until the founder provides it.
- Email list: has any of it been emailed before? What segments exist (WV vs LA, org size, contact role)?
- Does the 7-day free trial require a credit card? (No card = lead magnet; card = tripwire)
- Guarantee or refund terms
- Does the founder have grant-writing or nonprofit experience? (credibility for content)
